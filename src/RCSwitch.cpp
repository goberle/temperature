...
uv_async_t *RCSwitch::nAsync = NULL;
...
void RCSwitch::enableReceive(int interrupt, uv_async_t* async) {
  RCSwitch::nAsync = async;
...
void RCSwitch::handleInterrupt() {
  ...
  if (duration > 5000 && duration > RCSwitch::timings[0] - 200 && duration < RCSwitch::timings[0] + 200) {
    ...
    if (repeatCount == 2) {
      if (receiveProtocol1(changeCount) != false || receiveProtocol2(changeCount) != false) {
        uv_async_send(RCSwitch::nAsync);
      }
      repeatCount = 0;
    }
